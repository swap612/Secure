...
void CACHE::handle_fill()
{
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu])
    {
        ...
        if (cache_type == IS_LLC)
        {
            ...
            {
                int c = ooo_cpu[i].L2C.invalidate_entry(block[set][way].address);

                cout<<"\nSearch:"<<ooo_cpu[i].L2C.search_entry(block[set][way].address);
                cout<<"C: "<<c<<endl;
                if (c != -1)
                {
                    //increment invalidate count once for an address
                   // if (!foundInL2)
                    {
                        ooo_cpu[i].L2InvalidateCount++;
                        cout<<"Count"<< ooo_cpu[fill_cpu].L2InvalidateCount;
                        foundInL2 = 1;
                    }
                    // invalidate in L1

                    if(ooo_cpu[i].L1D.invalidate_entry(block[set][way].address) != -1)
                        cout<<endl<< ooo_cpu[fill_cpu].L2InvalidateCount++;;
                        // ooo_cpu[fill_cpu].L1InvalidateCount++;
                    if(ooo_cpu[i].L1I.invalidate_entry(block[set][way].address) != -1)
                        cout<<endl<< ooo_cpu[fill_cpu].L2InvalidateCount++;;
                        // ooo_cpu[fill_cpu].L1InvalidateCount++;
                }

                // cout<<"\n Log :: "<<block[set][way].tag << "  Full addr "<<block[set][way].full_addr;
                // cout << "\n L2C : "<<ooo_cpu[i].L2C.invalidate_entry(block[set][way].tag);
                // cout << "\n L1D : "<<ooo_cpu[i].L1D.invalidate_entry(block[set][way].tag);
                // cout << "\n L1I : "<<ooo_cpu[i].L1I.invalidate_entry(block[set][way].tag);
            }
        }
        else if (cache_type == IS_L2C)
        {
            ...
            // cout<<"\nSearch L1D:"<<ooo_cpu[fill_cpu].L1D.search_entry(block[set][way].address);
            // cout<<"\nSearch L1I:"<<ooo_cpu[fill_cpu ].L1I.search_entry(block[set][way].address);
            //invalidate the L1 cache
             if(ooo_cpu[fill_cpu].L1D.invalidate_entry(block[set][way].address) != -1)
                cout<<endl<< ooo_cpu[fill_cpu].L2InvalidateCount++;
            //    cout<< L1InvalidateCount++;
             if(ooo_cpu[fill_cpu].L1I.invalidate_entry(block[set][way].address) != -1)
                cout<<endl<< ooo_cpu[fill_cpu].L2InvalidateCount++;
                // cout<<L1InvalidateCount++;

             // cout << "\n L1D : "<<ooo_cpu[fill_cpu].L1D.invalidate_entry(block[set][way].address);
             // cout << "\n L1I : "<<ooo_cpu[fill_cpu].L1I.invalidate_entry(block[set][way].address);
        }
        else
            way = find_victim(fill_cpu, MSHR.entry[mshr_index].instr_id, set, block[set], MSHR.entry[mshr_index].ip, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].type);
...
int CACHE::invalidate_entry(uint64_t inval_addr)
{
    ...
    for (uint32_t way = 0; way < NUM_WAY; way++)
    {
        if (block[set][way].valid && (block[set][way].tag == inval_addr))
        {
            ...
            cout<<"\t tag:"<<block[set][way].tag <<"\t addr: "<<block[set][way].address <<"\t full addr: "<<block[set][way].full_addr<<endl;
            match_way = way;
